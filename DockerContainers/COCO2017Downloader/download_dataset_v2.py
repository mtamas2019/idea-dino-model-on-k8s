...
clean_dataset_dir = os.environ.get('CLEAN_DATASET_DIR', 'No')
...
#  Download full set of train pictures in zip format if percent is 100 % (downloaded in zip is much faster than individually
if download_percent == 100:
  ...
  if len(os.listdir(train_dir)) !=118287:
    ...
    # Start downloading
    logging.info('Start download full set of train pictures')
...
else: #Download subset of train images

  #Initalize COCO Dataset
   logging.info('Download subset of train images mode')
   ...
   logging.info("Start download training images")
   ...
   for category_id in category_ids:
     ...
     logging.info(f"Category: {category_id_to_name.get(category_id)}, Full dataset size: {len(image_ids)} images, Download size: {len(download_image_ids)} images")
...
# Write out the noe annotation file
